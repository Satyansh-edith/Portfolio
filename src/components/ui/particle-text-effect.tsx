...
class Particle {
  ...
  draw(ctx: CanvasRenderingContext2D, drawAsPoints: boolean) {
    ...
    if (drawAsPoints) {
      ...
      ctx.fillRect(this.pos.x - 1.25, this.pos.y - 1.25, 2.5, 2.5)
    } else {
      ctx.fillStyle = `rgb(${currentColor.r}, ${currentColor.g}, ${currentColor.b})`
      ctx.beginPath()
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    if (word.length > 12) fontSize = Math.round(canvas.height * 0.16)
    offscreenCtx.font = `800 ${fontSize}px 'Syne', sans-serif`
    ...
    // Generate new color from a high-contrast palette
    const colors = [
      { r: 0, g: 243, b: 255 },   // Electric Cyan
      { r: 168, g: 85, b: 247 },  // Cyber Violet
      { r: 16, g: 185, b: 129 },  // Emerald Green
      { r: 245, g: 158, b: 11 },  // Amber Glow
      { r: 255, g: 255, b: 255 }, // Plain White
  ...
  const animate = () => {
    ...
    // Clear background with high alpha transparency to allow WebGL behind it to render clearly
